...
public class JADE_MAS_AdapterAPI {
	...
	private void assignJadeAgentsToDistributedAutonomousAgents(
		...
		for (final NativeDistributedAutonomousAgent jadeAgent : jadeAgents) {
			assignJadeAgentToDistributedAutonomousAgent(jadeAgent,
					jade_MAS_RunContext);
		}
	}
	...
	public JADE_MAS_RunContext initializeSimulationRun(
		...
		jade_MAS_RunContext
		.setJadeContextForThisRun(nativeJadeContextForThisRun);
		...
		.initializeDistributedAutonomousAgents(
				nativeJadeContextForThisRun, nativeAgentsSet);
		...
		if (executeHandshake) {
			...
			.println("[JADE Controller Agent] Received framework message from the simulation engine: "
					+ XMLUtilities.convertDocumentToXMLString(
							msg.getDocument(), true));
			final FRAMEWORK_COMMAND fc = msg
