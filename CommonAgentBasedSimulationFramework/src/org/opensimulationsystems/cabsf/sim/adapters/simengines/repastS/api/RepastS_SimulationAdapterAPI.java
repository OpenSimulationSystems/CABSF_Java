...
public class RepastS_SimulationAdapterAPI {
	...
	public DefaultParameters applyRssrParametersFix(
		...
		final List<Element> parameters = (List<Element>) XMLUtilities
				.executeXPath(repastSconfigFile, "/parameters", null,
						elementFilter);
		...
		for (int i = 0; i < parameterElements.size(); i++) {
			...
			final String paramName = parameterElements.get(i)
					.getAttributeValue("name");
			final String displayName = parameterElements.get(i)
					.getAttributeValue("displayName");
			final String defaultValue = parameterElements.get(i)
			...
			if (defaultValue.equals("__NULL__")) {
				defaultParameters.addParameter(paramName, displayName,
						Number.class,
						// null, true);
						num, true);
					...
					.equals("int")) {
				defaultParameters.addParameter(paramName, displayName,
						Number.class, Integer.parseInt(defaultValue), true);
			} else if (parameterElements.get(i).getAttributeValue("type")
					.equalsIgnoreCase("String")) {
				defaultParameters.addParameter(paramName, displayName,
						String.class, defaultValue, true);
			}
		}
	...
	private void mapSimulationSideAgents(
			final Iterable<Object> agentsOfOneType,
			final SimulationRunContext simulationRunContext) {
	 ...
	 * presence of the flag in the configuration file.
	 *
