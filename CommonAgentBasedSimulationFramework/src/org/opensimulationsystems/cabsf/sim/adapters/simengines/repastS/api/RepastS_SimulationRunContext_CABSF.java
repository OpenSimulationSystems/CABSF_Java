 ...
 * Simulation-Toolkit-specific context mirrors the generic DistSysRunContext
 * provided by the Common Framework API. It enables API users to get native
 * Simulation-Toolkit objects instead of generic "Object"s. This aids the API
 * client at compile time. The simulation context is created for the entire
 * simulation run group, unlike in Repast where the simulation context exists
 * per simulation run. Adapter developers should first instantiate
 * DistSysRunContext, before instantiating a Simulation-Toolkit-specific Context
 * such as this class.
 *
...
public class RepastS_SimulationRunContext_CABSF {
    ...
    public SimulationDistributedSystemManager getSimulationDistributedSystemManager(
            final Object obj) {
        // FIXME: Return the right one/support multiple managers
        return simulationDistributedSystemManagers.iterator().next();
    }

    /**
     * Gets the simulation distributed system managers. Instead of this method
     * use getSimulationDistributedSystemManager(Object) to get the appropriate
     * distributed system manager for an agent.
     *
     * @deprecated
     * @return the simulation distributed system managers
     */
