...
/**
 * Manages the CRUD operations to Redis. This class has no business-level
 * knowledge.
 *
 *
...
public class RedisConnectionManager {
    ...
    public void postMessage(final String channel, final String message) {
        // actually using a Redis key for now, not a channel as used in
        // Publish/Subscribe
        jedis.lpush(channel, message);

        // LOW: Future work. Switch to Publish/Subscribe model using
        // multithreading for
        // the separate
     ...
     *            the maximum number of polls on the Redis key. If null, this
     *            will poll forever.
     * @return the string
     */
    public String redisSynchronousPolling(final SYSTEM_TYPE requestingSystemType,
        ...
        .println("["
                ...
                + "\r\nBoth processes must be running before issuing the start simulation command. "
                + "\r\nAfter the simulation execution, the simulation process terminates "
                + "\r\nbut the distributed system/MAS process continues to execute, waiting"
                + "\r\nfor possible additional simulation runs.");
