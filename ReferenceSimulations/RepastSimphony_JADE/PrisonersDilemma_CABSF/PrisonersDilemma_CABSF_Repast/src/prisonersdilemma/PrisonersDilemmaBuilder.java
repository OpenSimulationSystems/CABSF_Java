...
public class PrisonersDilemmaBuilder implements ContextBuilder<Object> {
    ...
    public Context build(final Context<Object> context) {
        ...
        final GameAdministrator gameAdministrator =
                new GameAdministrator();
        context.add(gameAdministrator);

        final Parameters params =
                RunEnvironment.getInstance().getParameters();
        final int playerCount =
                (Integer) params.getValue("player_count");
        if (playerCount % 2 != 0) {
            throw new CabsfRuntimeException(
                    "The CABSF only supports an even number of players for now.");
        }
        ...
        for (int i =
                0; i < playerCount; i++) {
            final Player player =
                    new Player(gameAdministrator);
            player.setPlayerNumber(i);
            context.add(player);
        }

        // Calculate the number of combinations
        final BigInteger nFact =
                BigIntegerMath.factorial(playerCount);
        final BigInteger nMinusRfact =
                BigIntegerMath.factorial(playerCount - 2);
        final BigInteger rFact =
                BigIntegerMath.factorial(2);
        final Number denominator =
                LongMath.multiply(nMinusRfact, rFact);
        final Number numCombinations =
                LongMath.divide(nFact, denominator);
        final int numbCombinationsInt =
                numCombinations.intValue();
