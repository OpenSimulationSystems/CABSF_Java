 ...
 * developer using the Common Agent-Based Simulation Framework. The ID should be globally
 * unique and specified in the configuration, therefore, it is not generated by the
 * framework. Each instance of the Simulation Run Group would have its own id generated by
 * the framework with each simulation group run, which is stored as a separate id from the
 * overall Simulation Run Group id.
 */
public class SimulationRunGroup {
	 ...
	 * the CABSF configuration, and supplied to this method. The optional
	 * simulationVersion allows the simulation authors to keep track of different versions
	 * of a simulation. simulationConfigurationID may be used for versioning of the
	 * Simulation Tool such as Repast (e.g. varying the number of agents or grid cells).
	 * commonSimulationFrameoworkConfigurationID may be used for versioning of CABSF
	 * configuration (such changing the number of simulations runs to execute or number of
	 * ticks for each simulation run). It is assumed that all simulation runs in a
	 * simulation run group have the same configuration.
	 *
