...
/**
 * The abstract parent of all context classes. This class provides common
 * functionality across the contexts, such a method to generate a
 * FrameworkMessage from the message exchange (CabsfMessage) template
 *
 * @author Jorge Calderon
 * @version 0.1
 * @since 0.1
 */
public abstract class AbstractContext_Cabsf {
    ...
    /**
     * Used by the distributed autonomous agents to convert the XML fragment in
     * the original incoming message back into a full Message Exchange XML, then
     * wrap it in a FrameworkMessage. The purposes of this is to give the agent
     ...
     * @param sourceSystem
     *            the source system. Can be null if only creating the
     *            FrameworkMessage to use the convenience methods and not
     *            actually sending this FrameworkMessage across the wire.
     * @param targetSystem
     *            the target system. Can be null if only creating the
     *            FrameworkMessage to use the convenience methods and not
     *            actually sending this FrameworkMessage across the wire.
     ...
     */
    public void setupCachedMessageExchangeTemplate(
    ...
    public void setupElementTemplates(final Document doc) {
        ...
        final List<Element> distributedAutonomousAgentElements = (List<Element>) XMLUtilities
                .executeXPath(
                        doc,
                        "/x:CabsfMessage/x:MessageRecipients/x:DistributedSystem/x:DistributedSoftwareAgents/x:DistributedSoftwareAgent",
